...
void efi_assert(const char *expr, const char *file, unsigned line, const char *function) {
        ...
        /* Let's be paranoid. */
        if (asserting)
                panic(u"rebirth: Nested assertion failure, halting.");

        asserting = true;
        log_error("rebirth: Assertion '%s' failed at %s:%u@%s, halting.", expr, file, line, function);
        freeze();
}
...
void __stack_chk_fail(void) {
        panic(u"rebirth: Stack check failed, halting.");
}
...
void abort(void) {
        panic(u"rebirth: Unknown error, halting.");
}
...
int __aeabi_idiv0(int return_value) {
        panic(u"rebirth: Division by zero, halting.");
}
